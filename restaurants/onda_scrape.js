...
async function scrape_onda(url, date_index) {
    ...
    const [buffet_price_xpath] = await page.$x('//*[@id="lunchbox"]/div/div[3]/div[5]/div[2]/p');
    const buffet_price = await format_data(buffet_price_xpath)
    
    const [buffet_salad_price_xpath] = await page.$x('//*[@id="lunchbox"]/div/div[3]/div[3]/div[2]/p');
    const buffet_salad_price = await format_data(buffet_salad_price_xpath)


    const [lunch_option_1_price_xpath] = await page.$x('//*[@id="lunchbox"]/div/div[3]/div[6]/div[2]/p');
    ...
    const onda_buffet_salad = {"name": "Salaatti buffet", "price": buffet_salad_price + ' €'}
    const onda_lunch_vegan = {"name": lunch_option_1_name, "price": lunch_option_1_price + ' €'}
    const onda_lunch_meat = {"name": lunch_option_2_name, "price": lunch_option_2_price + ' €'}
    const onda_buffet_vegan = {"name": buffet_vegan_name, "price": buffet_price + ' €'}
    const onda_buffet_meat = {"name": buffet_meat_name, "price": buffet_price + ' €'}

    return [onda_buffet_salad, onda_lunch_vegan, onda_lunch_meat, onda_buffet_vegan, onda_buffet_meat]

}

module.exports = {scrape_onda};